  ...
  <div class="page">
    <div class="page-header">
      <div>
        ...
        <p class="description">Perbarui informasi kategori pelanggaran.</p>
      </div>
    </div>
    ...
    <div v-else-if="loadError" class="alert error-alert">
      <AlertCircle :size="18" />
      <span>{{ loadError }}</span>
    ...
    <form v-else class="form-card" @submit.prevent="submit">
      <div v-if="error" class="form-alert">
        <AlertCircle :size="18" />
        <span>{{ error }}</span>
      </div>
      ...
      <div class="section">
        <div class="section-heading">
          ...
          <p>Perbarui informasi kategori pelanggaran di bawah.</p>
        </div>
...
const fetchCategory = async () => {
  ...
  try {
    ...
    if (!category) {
      loadError.value = "kategori pelanggaran tidak ditemukan.";
      return;
    }

    form.nama_pelanggaran = category.nama_pelanggaran || "";
    form.poin = category.poin ?? null;
    form.kategori = category.kategori || "";
    ...
    loadError.value =
      err.response?.data?.message || "data kategori pelanggaran gagal dimuat.";
  } finally {
    loading.value = false;
  }
};
...
const submit = async () => {
  ...
  try {
    await api.put(`/staff/violation-categories/${route.params.id}`, {
    ...
    if (err.response?.status === 422) {
      ...
      error.value =
        err.response?.data?.message || "kategori pelanggaran gagal diperbarui.";
    }
  } finally {
